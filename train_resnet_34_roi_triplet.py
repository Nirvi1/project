...
from src.model import ResNet18Model, ResNet101RoI, get_loss_NCE, get_acc
...
from src.Resnet101Roi.run import  eval, train_roi
...
for epoch in range(curr_epoch, opt.optim.epochs):
    loss_tra, acc_tra = train_roi(model, dl_tra, optimizer, verbose, device, triplet_loss=True)
    ...
    loss_val, acc_val = eval(model, dl_val, optimizer, verbose, device, triplet_loss=True)
