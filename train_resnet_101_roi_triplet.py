...
for epoch in range(curr_epoch, opt.optim.epochs):
    loss_tra, acc_tra = train_roi(model, training_data_loader, optimizer, verbose, device, triplet_loss=True)
    ...
    loss_val, acc_val = eval(model, validation_loader, optimizer, verbose, device, triplet_loss=True)
