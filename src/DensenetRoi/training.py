...
def train_roi(model, training_data_loader, optimizer, verbose=0, device='cpu', cross=False):
    if device == 'cpu':
        model.cpu()
    else:
        model.cuda()
    ...
    for d in training_data_loader:
        ...
        if verbose >= 3:
            acc = corr / 6 / inputs.size(0)
            # print(f'  Batch Train:  Loss: {loss:.4f},  Acc: {acc:.4f}')
            print('  Batch Train:  Loss: {:.4f},  Acc: {:.4f}'.format(loss, acc))

    epoch_loss = running_loss / running_total
    epoch_acc = float(running_corrects) / 6 / running_total
    return epoch_loss, epoch_acc

def eval(model, validation_loader, optimizer, verbose=0, device='cpu', cross=False):
    if device == 'cpu':
        model.cpu()
    else:
        model.cuda()
